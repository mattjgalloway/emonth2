...
const DeviceAddress EXT_SENSOR1 = {
  0x28, 0x89, 0x7E, 0x8A, 0x06, 0x00, 0x00, 0xBE };
const DeviceAddress EXT_SENSOR2 = {
  0x28, 0x9B, 0x07, 0x8A, 0x06, 0x00, 0x00, 0x15 };
  
boolean EXT_SENSOR1_FOUND = 0;
boolean EXT_SENSOR2_FOUND = 0;
    ...
    if (debug==1) {
      ...
      for (int i=0; i<numSensors; i++) {
        ...
        if (sensors.getAddress(tempDeviceAddress, i)) {
          ...
          if (addressEqual(tempDeviceAddress, EXT_SENSOR1)) {
            EXT_SENSOR1_FOUND = 1;
            Serial.print(" (Sensor 1)");
          } else if (addressEqual(tempDeviceAddress, EXT_SENSOR2)) {
            EXT_SENSOR2_FOUND = 1;
          ...
          Serial.print("Failed to query device!");
        }
      }
      
      if (!EXT_SENSOR1_FOUND) {
        Serial.print("Didn't find sensor 1");
      }
      if (!EXT_SENSOR2_FOUND) {
        Serial.print("Didn't find sensor 2");
      }
    }
  }
...
//################################################################################################################################
{
  ...
  if (WDT_number>=WDT_MAX_NUMBER || pulseCount>=PULSE_MAX_NUMBER)
  {
    ...
    if (DS18B20==1)
    {
      ...
      if (EXT_SENSOR1_FOUND && (temp1<125.0) && (temp1>-40.0))
      {
        emonth.temp_external1=(temp1*10);
      }
      if (EXT_SENSOR2_FOUND && (temp2<125.0) && (temp2>-40.0))
      {
    ...
    // Works with EmonESP direct serial
    {
      ...
      if (DS18B20){
        Serial.print("tempex1:");Serial.print(emonth.temp_external1); Serial.print(",");
        Serial.print("tempex2:");Serial.print(emonth.temp_external2); Serial.print(",");
      }
...
boolean addressEqual(DeviceAddress lhs, DeviceAddress rhs)
{
  for (uint8_t i = 0; i < 8; i++)
  {
